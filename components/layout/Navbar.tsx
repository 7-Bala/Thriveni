    ...
    <>
      ...
      <header className="fixed w-full top-0 z-50 flex flex-col">
        ...
        <div className="h-10 bg-metal-900 text-metal-100 font-body text-[10px] uppercase tracking-[0.2em] hidden lg:flex items-center justify-between px-10 xl:px-16 border-b border-white/5">
          <div>Chennai&apos;s No.1 Multi-Brand Dealership</div>
        ...
        <div className={`h-[80px] transition-all duration-500 ease-in-out ${navBg}`}>
          <div className="container-custom h-full flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center gap-10 h-full" ref={dropdownRef}>
              {NAV_LINKS.map((link) => {
                ...
                return (
                  ...
                  >
                    <Link 
                      href={link.href}
                      className={`font-body text-[13px] font-medium tracking-wide transition-colors ${isActive ? 'text-amber-cta' : 'text-metal-100 group-hover/nav:text-amber-cta'}`}
                    >
                      {link.label}
                      
                      {/* Premium Underline Animation */}
                      <motion.span 
                        className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-cta origin-left"
                        initial={{ scaleX: 0 }}
            ...
            {/* Desktop Actions - Anti-AI button rounded-sm */}
            <div className="hidden lg:flex items-center gap-6">
              <button className="text-metal-100 hover:text-amber-cta p-2 transition-all hover:rotate-12">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
              </button>
              <Link 
                href="/contact" 
                className="bg-amber-cta text-white font-body font-bold text-[11px] uppercase tracking-[0.2em] px-8 py-3 rounded-sm hover:bg-amber-light transition-all active:scale-95 shadow-lg"
              >
                Book Test Drive
              </Link>
            </div>
          ...
          >
            <div className="flex flex-col gap-8">
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <Link 
                    href={link.href}
                    className={`font-display text-4xl uppercase tracking-wider ${pathname === link.href ? 'text-amber-cta' : 'text-white'}`}
                  >
                    {link.label}
                  </Link>
                  {link.dropdown && (
                    <div className="flex flex-col gap-4 mt-4 pl-6 border-l border-white/5">
                      {link.dropdown.map((sublink) => (
                        <Link 
                          key={sublink.label} 
                          href={sublink.href}
                          className="text-metal-400 text-xl font-body hover:text-amber-cta transition-colors"
                        >
                          {sublink.label}
                        </Link>
                      ))}
            ...
            >
              <div className="flex justify-between items-center text-metal-300">
                <a href="tel:+919876543210" className="flex items-center gap-3 text-lg font-medium">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                  Call Us
                </a>
                <a href="https://wa.me/919876543210" className="flex items-center gap-3 text-[#25D366] text-lg font-medium">
